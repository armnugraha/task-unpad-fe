...
if (isset($_POST['mapelSave'])) {
	$schedule = mysqli_query($con,"SELECT * FROM schedules WHERE id_mapel='$_POST[mapel]'") or die(mysqli_error($con)) ;
  $data = mysqli_fetch_array($schedule);
  $schedule_id = $data ? $data["id"]:null;
  $sql= mysqli_query($con,"INSERT INTO tb_roleguru VALUES (NULL,'$_POST[id_guru]','$_POST[kelas]','$_POST[mapel]','$_POST[semester]','$_POST[jurusan]','$schedule_id') ") or die(mysqli_error($con));
  if ($sql) {
		echo "
...
echo '<div class="error">ERROR: Ekstensi file tidak di izinkan!</div>';
  ...
  elseif (isset($_POST['materiUpdate'])) {
    ...
    $sql= mysqli_query($con,"UPDATE tb_guru SET nama_guru='$_POST[nama]',email='$_POST[email]',password='$password' WHERE id_guru='$_POST[ID]' ") or die(mysqli_error($con));
      if ($sql) {
		echo "
			<script type='text/javascript'>
